...
for course_dir in os.listdir(content_root):
    ...
    # Get all base filenames from content/course/*.json
    # Ignore subdirectories like prerequisites/ and explanations/
    json_files = [f for f in os.listdir(course_path) if f.endswith(".json")]

    for json_file in json_files:
        base = json_file.replace(".json", "")
        metadata_path = os.path.join(course_path, json_file)
        ...
        svg_files = []
        # Typst output is currently flat in dist/content
        # Let's keep it that way for now or adjust build.sh
